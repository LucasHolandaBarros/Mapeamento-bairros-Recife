...
class Graph:
    """
    Implementação de um Grafo não-direcionado usando lista de adjacência.
    
    A estrutura da lista de adjacência é um dicionário onde:
    - Chave: str (nome do nó, ex: 'boa viagem')
    - Valor: dict (vizinhos)
        - Chave (vizinho): str (nome do nó vizinho, ex: 'ipsep')
        - Valor (atributos da aresta): dict (ex: {'weight': 1.0, 'logradouro': '...'})
    ...
    Também armazenamos os atributos dos nós (ex: microrregião) em 'self.nodes'.
    self.nodes = {
        'boa viagem': {'microrregiao': '1.1'},
        'ipsep': {'microrregiao': '1.2'},
        ...
    }
    """
